...
### 1.4 图标库: lucide-react

- **选型理由**:
  - **最简单**: 轻量级且按需加载，提供丰富的线条风格图标，完美契合设计图中的视觉需求（如回到顶部的箭头图标、可能的筛选图标等），即插即用。

---

## 2. 样式与移动端优先适配方案 (Mobile First)

**核心理念：本项目严格遵循“移动端优先”原则。所有的 UI 尺寸、排版和交互都完全以手机屏幕（如 iPhone 的 375px/390px 宽度）为基准进行设计和开发，不考虑 PC 端的响应式展开。**

### 2.1 CSS 方案: Tailwind CSS

- **选型理由**:
  - **最简单**: Utility-first 的理念允许你直接在 JSX 中编写样式，非常适合快速还原移动端的复杂 UI（如多重标签、微小间距）。
  - **最健壮**: 生产环境自动剔除未使用的 CSS，打包体积极小，且彻底杜绝了 CSS 全局污染和类名冲突问题。
  - **移动端友好**: Tailwind 默认支持移动端优先的媒体查询（虽然我们主要只写移动端样式，但在外层容器限制最大宽度时非常方便）。

### 2.2 移动端适配与布局策略

- **布局方案**: 全面采用 Flexbox。对于移动端的侧边栏与商品流布局，Flex 具有最强的灵活性和最健壮的自适应能力。
- **单位与缩放**: 推荐使用 `vw` (视口宽度) 作为核心单位（或配置 `postcss-px-to-viewport` 插件），确保字体、图片和间距在不同尺寸的手机上能等比缩放。
- **PC 端兼容策略**: 既然是移动端优先，当用户在 PC 浏览器打开时，**绝不进行响应式全屏拉伸**。而是在最外层容器设置 `max-width: 480px; margin: 0 auto;`，使其在 PC 上呈现为一个居中的“手机模拟器”外观，保证核心布局不被破坏。
